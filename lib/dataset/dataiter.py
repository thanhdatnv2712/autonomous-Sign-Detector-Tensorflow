...
class FaceBoxesDataIter():
    ...
    def _map_func(self,dp,is_training):
        ...
        for i in range(boxes.shape[0]):
            ...
            if (box[3] - box[1]) * (box[2] - box[0]) < cfg.DATA.cover_small_face:
                ...
                boxes_clean.append([box[1]/cfg.MODEL.hin, box[0]/cfg.MODEL.win, box[3]/cfg.MODEL.hin, box[2]/cfg.MODEL.win, box[4]]) # convert xyxy to yxyx
        boxes = np.array(boxes_clean)

        if cfg.TRAIN.vis:
            for box in boxes:
                cv2.rectangle(image, (int(box[1]*cfg.MODEL.win), int(box[0]*cfg.MODEL.hin)), (int(box[3]*cfg.MODEL.win), int(box[2]*cfg.MODEL.hin)), (255, 0, 0), 2)
                cv2.putText(image, str(int(box[4])), (int(box[1]*cfg.MODEL.win), int(box[0]*cfg.MODEL.hin)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,0,0), 2)
    ...
    def produce_target(self,bboxes):
        reg_targets, cls_targets = get_training_targets(bboxes, threshold=cfg.MODEL.MATCHING_THRESHOLD)
