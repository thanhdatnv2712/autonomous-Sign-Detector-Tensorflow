...
class FaceBoxesHead(tf.keras.Model):
    def __init__(self,
        ...
        self.conv_reg_1 = tf.keras.layers.Conv2D(filters=4 * 21,
                                                 ...
                                                 use_bias=True,
                                                 kernel_initializer=kernel_initializer)
        self.conv_cls_1 = tf.keras.layers.Conv2D(filters=cfg.MODEL.num_classes * 21,
                                                 kernel_size=(3, 3),
                                                 strides=1,
                                                 padding='same',
                                                 use_bias=True,
    ...
    def __call__(self, fms, training=False):
        ...
        cls_1 = tf.reshape(cls_1, ([-1, 32, 32, 21, cfg.MODEL.num_classes]))
        cls_1 = tf.reshape(cls_1, ([-1, 32 * 32 * 21, cfg.MODEL.num_classes]))
...
class FaceBoxes(tf.keras.Model):
    ...
    def get_predictions(self,box_encodings,cla,anchors, score_threshold=cfg.TEST.score_threshold, iou_threshold=cfg.TEST.iou_threshold, max_boxes=cfg.TEST.max_boxes):
        ...
        for i in range(1, cfg.MODEL.num_classes):
            scores_i = tf.nn.softmax(cla, axis=2)[:, :, i]
            with tf.device('/cpu:0'):
                boxes_i, scores_i, num_boxes_i = batch_non_max_suppression(
                    boxes, scores_i, score_threshold, iou_threshold, max_boxes
                )
            list_boxes.append(boxes_i)
            list_scores.append(scores_i)
            list_num_boxes.append(num_boxes_i)
        
        list_boxes = tf.convert_to_tensor(list_boxes)
        list_scores = tf.convert_to_tensor(list_scores)
        list_num_boxes = tf.convert_to_tensor(list_num_boxes)

        boxes=tf.identity(list_boxes,name='boxes')
        scores = tf.identity(list_scores, name='scores')
        num_boxes = tf.identity(list_num_boxes, name='num_boxes')

        return {
            'boxes': list_boxes, 
            'scores': list_scores, 
            'num_boxes': list_num_boxes,
        }
...
@tf.function
def calculate_loss(reg_targets, cls_targets, loc_predict, cls_predict):
    #### loss

    # whether anchor is matched
    is_matched = tf.greater(cls_targets, 0)
    ...
    cls_targets = tf.cast(cls_targets,dtype=tf.int32)
...
if __name__=='__main__':
    input=tf.zeros(shape=[1,cfg.MODEL.hin, cfg.MODEL.win,3],)

    model=FaceBoxes()

    x=model.inference(input)
