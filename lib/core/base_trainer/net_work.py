...
from lib.core.model.facebox.net import calculate_loss
...
class Train(object):
  ...
  def compute_loss(self, reg_targets, cls_targets, loc_predictions, cls_predictions, training):
    loss = tf.reduce_sum(calculate_loss(reg_targets, cls_targets, loc_predictions, cls_predictions))
  ...
  def train_step(self, inputs):
    ...
    Returns:
      loss: Scaled loss.
    """
    image, reg_targets, cls_targets = inputs
    with tf.GradientTape() as tape:
      loc_predictions,cls_predictions = self.model(image, training=True)

      loss = self.compute_loss(reg_targets, cls_targets, loc_predictions, cls_predictions, True)
  ...
  def custom_loop(self, train_dist_dataset, test_dist_dataset,
    ...
    for epoch in range(self.epochs):
      ...
      current_model_saved_name=os.path.join(cfg.MODEL.model_path,
                                            'epoch_%d_val_loss%.6f'%(epoch,train_total_loss / num_train_batches))
